<?php
    ...
    //session_destroy();
    //unset('dangnhap');
    if(isset($_SESSION['name_admin'])  && isset($_SESSION['id_admin']) )
        {
        ...
        if($taikhoan =='' || $matkhau ==''){
            // nhap du thong tin
             ...
             $sql = "SELECT * FROM user WHERE userName ='".$taikhoan."' AND password = '".$matkhau."' and role = 1 limit 1";
            
            $query = mysqli_query($conn,$sql);
            $count = mysqli_num_rows($query);
            $row_login = mysqli_fetch_array($query);
            if($count >0){           
                $_SESSION['name_admin'] = $row_login['fullName'];
                $_SESSION['id_admin'] = $row_login['role'];
                unset($_SESSION['idnguoidung']);
                unset($_SESSION['dangnhap']);
                header("Location:dashboard.php");
            }else{
